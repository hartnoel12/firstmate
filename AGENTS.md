...
config/verify/<project>  optional per-project declaration of what a complete verification is, as `<step> = <command>` lines; LOCAL, gitignored; declared steps become required at merge time (docs/configuration.md "Merge verification evidence"); not inherited by secondmate homes
...
state/               volatile runtime signals; gitignored
  ...
  <id>.verification  append-only per-task ledger of verification runs, authorized bypasses, and taken merge overrides; written by fm-verify.sh and by the merge override, read by the merge gate (bin/fm-verify-lib.sh)
...
For PR-based ship tasks, the ready signal depends on mode: `no-mistakes` reports `done: PR <url> checks green` only once CI genuinely reports every check green, or `done: PR <url>` plus the exact CI state it actually observed otherwise (never "checks green" as boilerplate), while `direct-PR` reports `done: PR <url>` after opening the PR.
