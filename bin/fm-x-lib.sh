...
# Static body of config/x-mode.env. Kept in a function body rather than inline in
# `VAR=$(cat <<EOF)` at the call site: bash 3.2 cannot parse a heredoc nested in a
# command substitution, so one apostrophe in these comments would break the whole
# caller. See bin/fm-bash-syntax-check.sh.
fmx_cadence_content() {
  cat <<'EOF'
