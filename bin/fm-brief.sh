...
# Prose-bearing heredocs live in functions, never inline in `VAR=$(cat <<EOF)`.
# Bash 3.2 - the system bash on macOS - cannot parse a heredoc body nested in a
# command substitution, so one apostrophe in the prose breaks the whole script.
# See bin/fm-bash-syntax-check.sh for the full rule. Call the function instead.
herdr_section_not_enabled() {
  cat <<'EOF'
...
HERDR_SECTION=$(herdr_section_not_enabled)
fi
