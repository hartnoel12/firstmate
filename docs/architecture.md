...
Once firstmate has been shown a crew idle under its standing `paused:` or `captain-held` line, by that surfaced signal, a surfaced turn end, or one stale wake, the window is parked and the pause path alone owns both its stale wakes and any further bare turn-end signal, absorbing each silently (outside away mode) unless the pane is busy.
...
Both that helper and the local-only fast-forward path refuse to land a commit firstmate holds no local verification evidence for, bound to the exact commit each would merge - the forge-reported PR head, or the branch tip; [configuration.md](configuration.md) "Merge verification evidence" owns the operator-facing contract and `bin/fm-verify-lib.sh` owns the gate rules, the recorded bypass, and the deliberately awkward override.
