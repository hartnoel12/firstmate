...
- Never expand a possibly-empty array bare (`"${a[@]}"`, `"${a[*]}"`); write `${a[@]+"${a[@]}"}` or test `"${#a[@]}"` first, because Bash 3.2 makes the bare expansion of an empty array a fatal unbound-variable error under `set -u` while Bash 5 expands it to nothing.
- Enforcement is split the same way: `tests/fm-bash32-empty-array.test.sh` bans the unguarded accumulator shape structurally across `bin/` on every lane and additionally runs the real crash when a Bash 3.2 interpreter is available (the macOS CI lane, or `FM_BASH32` locally); `docs/verification/bash32-empty-array.md` owns the measurements behind both halves.
