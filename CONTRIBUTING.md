...
```sh
bin/fm-bash-syntax-check.sh   # parse-check every shell root; on macOS this is the Bash 3.2 the fleet runs
...
CI owns broad regression across required portable parallel shards, the portable serial lane, the Herdr lane, lint, invariants, the coverage guard, and the stock macOS Bash 3.2 lane that parse-checks every canonical shell root, runs the Bash 3.2 empty-array runtime guard, and runs the snapshot compatibility tests, in [`.github/workflows/ci.yml`](.github/workflows/ci.yml).
