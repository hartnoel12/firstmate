...
CI owns broad regression across required portable parallel shards, the portable serial lane, the Herdr lane, lint, invariants, the coverage guard, and the stock macOS Bash 3.2 lane that both parse-checks every canonical shell root and runs the snapshot compatibility tests, in [`.github/workflows/ci.yml`](.github/workflows/ci.yml).
