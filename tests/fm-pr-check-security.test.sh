...
test_valid_recording_and_merge_derivation() {
  ...
  : > "$dir/gh-axi.log"
  # bin/fm-pr-merge.sh refuses a head it holds no local verification evidence
  # for (bin/fm-verify-lib.sh). This case is about repository derivation and
  # merge method, so give it the record the gate requires;
  # tests/fm-merge-verification.test.sh owns the gate's own coverage.
  printf 'verify\t%s\t%s\tpassed\ttest:passed\t-\n' "$(date +%s)" "$expected" \
    > "$dir/home/state/task-a.verification"
  chmod 600 "$dir/home/state/task-a.verification"
  ...
  printf 'verify\t%s\t%s\tpassed\ttest:passed\t-\n' "$(date +%s)" \
    0123456789abcdef0123456789abcdef01234567 > "$dir/home/state/Task_A.1.verification"
  chmod 600 "$dir/home/state/Task_A.1.verification"
...
SH
    ...
    printf 'verify\t%s\t%s\tpassed\ttest:passed\t-\n' "$(date +%s)" \
      0123456789abcdef0123456789abcdef01234567 > "$dir/home/state/$id.verification"
    chmod 600 "$dir/home/state/$id.verification"
