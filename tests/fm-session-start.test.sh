...
EOF
  ...
  # gh-axi rather than a system tool: BASE_PATH includes /usr/bin, so removing
  # the stub for anything a host might install there (node, git, gh) leaves the
  # real one findable and no diagnostic is produced. gh-axi is firstmate's own
  # tool and never lands on a base PATH.
  rm -f "$fakebin/gh-axi"
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # gh-axi rather than a system tool, for the reason given in the ordering case.
  rm -f "$fakebin/gh-axi"
